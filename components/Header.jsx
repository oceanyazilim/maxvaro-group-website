...
export default function Header() {
  ...
  // Burger button — transparent, lines adapt to background
    ...
    <>
      ...
      >
        <Container className="flex items-center justify-between h-[72px] md:h-[88px]">
          <Link href="/" className="relative flex items-center gap-3 z-10" aria-label={site.name}>
          ...
          {/* Mobile burger — transparent, sadece çizgiler */}
          <button
            type="button"
            ...
            className="lg:hidden relative z-[60] -mr-1 inline-flex items-center justify-center h-12 w-12 touch-manipulation"
          >
            <span className="sr-only">Menü</span>
            <div className="relative h-4 w-6">
              <span className={`absolute left-0 right-0 h-[1.5px] rounded transition-all duration-300 ${burgerLine} ${mobileOpen ? "top-1/2 -translate-y-1/2 rotate-45" : "top-0"}`} />
              <span className={`absolute left-0 right-0 h-[1.5px] rounded transition-all duration-300 ${burgerLine} ${mobileOpen ? "opacity-0" : "top-1/2 -translate-y-1/2"}`} />
              <span className={`absolute left-0 right-0 h-[1.5px] rounded transition-all duration-300 ${burgerLine} ${mobileOpen ? "top-1/2 -translate-y-1/2 -rotate-45" : "bottom-0"}`} />
            </div>
          </button>
        </Container>
      ...
      {/* ===== MOBILE MENU ===== rendered outside <header> for clean stacking */}
      {/* Backdrop */}
      <div
        onClick={() => setMobileOpen(false)}
        aria-hidden="true"
        className={`lg:hidden fixed inset-0 z-[55] bg-ink-900/50 backdrop-blur-sm transition-opacity duration-400 ease-smooth
      ...
      <aside
        ...
        className={`lg:hidden fixed top-0 right-0 bottom-0 z-[58] w-[88%] max-w-[420px] bg-white shadow-cardHover
        ...
        <div className="flex items-center justify-between h-[72px] px-5 border-b border-ink-100">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col leading-none">
              <span className="font-display text-[15px] font-semibold tracking-wide text-ink">MAXVARO</span>
              <span className="text-[9px] tracking-widest2 text-ink-400 mt-1">GROUP</span>
            </div>
          </div>
          <button
            type="button"
            ...
            className="grid place-items-center h-10 w-10 -mr-2 text-ink-500 hover:text-ink touch-manipulation"
          >
                    ...
                    {hasDropdown ? (
                      <>
                        <div className="flex items-stretch">
                          <Link
                            href={item.href}
                            className={`flex-1 py-4 ${active ? "text-ink" : "text-ink-700"}`}
                          >
                            ...
                            {dropdownData[item.dropdown].items.map((sub) => {
                              ...
                              return (
                                <li key={sub.href}>
                                  <Link
                                    href={sub.href}
                                    className={`flex items-center justify-between gap-3 py-2.5 pl-4 border-l-2 transition-colors
                      ...
                      >
                        <span className="font-display text-xl">{item.label}</span>
                        <span className={`transition-all duration-300 ${active ? "text-gold opacity-100" : "text-ink-300 group-hover:text-gold"}`}>
